...
mckinze= read.csv("C:\\Users\\ashle\\Downloads\\mckinze_reviews_csv.csv", header = TRUE)
colnames(mckinze)

mckinze= mckinze%>% 
  gather(pros, cons, key = "review_sentiment", value = "review",-heading)
summary(mckinze$review_sentiment)

mckinzereviews = mckinze%>%

  select(review,review_sentiment)

mckinzereviews $review= as.character(mckinzereviews $review)

 

tidy_dataset = mckinzereviews %>%
