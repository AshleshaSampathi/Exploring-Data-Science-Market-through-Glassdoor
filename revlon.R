...
revlon= read.csv("C:\\Users\\ashle\\Downloads\\rev_reviews_csv.csv", header = TRUE)
colnames(revlon)

revlon= revlon%>% 
  gather(pros, cons, key = "review_sentiment", value = "review",-heading)
summary(revlon$review_sentiment)

revlonreviews = revlon%>%

  select(review,review_sentiment)

revlonreviews $review= as.character(revlonreviews $review)

 

tidy_dataset = revlonreviews %>%
